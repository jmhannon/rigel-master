...
{
    char ws[1024];
    // put fifos in /usr/comm
    (void)sprintf(ws, "/usr/comm/%s.in", name);
    telfixpath(ws, ws); // this does not do anything right now ??
    (void)unlink(ws);   // gets rid of any existing fifo
    ...
    (void)sprintf(ws, "/usr/comm/%s.out", name);
    telfixpath(ws, ws);
    (void)unlink(ws);
    if (mknod(ws, S_IFIFO | 0660, 0) < 0)
    {
