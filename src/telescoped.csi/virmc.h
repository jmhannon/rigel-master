...
typedef struct
{
    ...
    long    trackStart;         // ms time this path starts at
...
extern int vmcGetTrackingOffset(int node);
extern void vmcJog(int node, int amt);
extern void vmcStop(int node);
extern int vmcSetTrackPath(int node, int num, long startMs, int ivalMs, double *path);
