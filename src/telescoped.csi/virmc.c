...
/*
// also found in libastro, but that's not linked here
static void smallsleep(double t)
{
    struct timeval tv;
...
void vmcSetTargetPosition(int node, long position)
{
	...
	if (node == 2)
	{
		TRACE "vmcSetTargetPosition %d: %ld\n",node, position);
	}
...
void vmcResetClock(int node)
{
    ...
    vmcStop(node); // resetting the clock stops motion (per CSIMC doc)
...
// Get the tracking offset
int vmcGetTrackingOffset(int node)
{
    VCNodePtr pvc = &vmcNode[node];
    return pvc->toffset;
}
...
int vmcSetTrackPath(int node, int num, long startMs, int ivalMs, double *path)
{
	int i;
	double scale;
	...
	TRACE "vmcSetTrackPath %d, %d items at %ld, %d apart\n",node,num,startMs,ivalMs);
...
static int oTrackProgram(VCNodePtr pvc)
{
	...
	if (span >= pvc->timeout)
	{
		TRACE "Timed out during tracking at %d ms\n", pvc->timeout);
		// stop
		pvc->velocity = 0;
		pvc->targetPos = pvc->currentPos;
		pvc->tracking = 0;
	...
	span = now - (pvc->trackStart + pvc->trackIval * i);
	...
	//    TRACE "Track point %d @ span=%ld, pos = %ld\n",i,span,pvc->targetPos);
